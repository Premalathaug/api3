...
function sateliteDetails() {
    const url = "https://isro.vercel.app/api/customer_satellites";//isro api
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            data.customer_satellites.forEach((element) => {   //for each
                const card = document.createElement("div");
                card.setAttribute("class", "card");
                card.setAttribute("style", "width: 18rem;");
                card.innerHTML += `
                <div class="card-body">
                  <h4 class="card-title">${element.country}</h4>
